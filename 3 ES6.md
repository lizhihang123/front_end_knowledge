...
3. Promsie，他的原型上有then和catch方法，自己身上有all reject resolve方法



## 4.2 promise特点

1. promise的状态pending、fulfilled、rejected，由异步操作的结果决定，外部无法决定。
2. 状态不可逆，pending -> fullfilled; pending -> rejected
3. 能够解决回调地狱的问题
...
new Promise()里面跟的是一个函数，

我们会获得resolve和reject两个参数，
...
**then()，是 promise上的原型的方法**，接受两个回调函数，第一个是成功后执行，第二个是失败才会执行的	  
...
>then的第二个回调函数和catch()会有什么区别？
>
>1. 一旦执行了result,状态是fulfilled，如果里面的出错了，error是捕获不到的，但是跟一个.catch()也能够捕获到成功里面的回调 下面的代码能够很清楚的看出区别，一个是直接报错，一个是打印错误信息
...
- 用来执行rejected的回调
- 如果resolve的回调出错了，也不会卡死，而是能够捕捉到错误

### 4. all()

等待所有的异步操作全部执行完毕，再执行下一步的.then操作。

把所有的promise对象，放进一个数组，.all方法接受一个数组，

r1, r2, r3和promise.all的顺序是几乎一样的。
...
- 和all很像，接受一个数组，里面是promise对象
- 返回值也是数组

差别：

1. 不管有没有报错，都会返回数组，里面的值 {status: 'rejected', value: 'xxx'； status: 'fulfilled', value: 'xxx'}；

2. 如果是all方法，如果有错误，就会报错。无法获得其他的报错的数据
...
比如，r1的结果第一个执行好，就打印r1。如果r1的结果是fulfilled，就打印fulfilled。否则就是rejected
